...
class AuthServer(pyraknet.server.Server):
	...
	def handle_login(self, data: bytes, address):
		...
		if (login):
			...
			player_service.add_account(account_id=user_info["AccountID"])
			session_service.add_session(address=address, user_key=user_key, account_id=user_info["AccountID"], username=username)

	def handle_packet(self, data : bytes, address : pyraknet.messages.Address):
		game.trigger_event("OnPacket_Auth_{}".format(str(data[0:8])), args=[data[8:], address])
